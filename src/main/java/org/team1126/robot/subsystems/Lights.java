...
public final class Lights {
    ...
    public Lights() {
        AddressableLED tmpLights = null;
        AddressableLEDBuffer tmpBuffer = new AddressableLEDBuffer(LENGTH * COUNT);
        boolean ok = false;
        try {
            // Attempt to open the configured PWM port for the LED strip.
            tmpLights = new AddressableLED(RioIO.LIGHTS);
            tmpLights.setLength(tmpBuffer.getLength());
            tmpLights.start();
            ok = true;
        } catch (Throwable t) {
            // If the port is invalid or already in use, gracefully disable lights usage.
            ok = false;
            tmpLights = null;
        }
    ...
    public void update() {
        if (available && lights != null) {
            lights.setData(buffer);
        }
    }

    /**
     * Returns true if the LED controller successfully initialized on the configured port.
     */
